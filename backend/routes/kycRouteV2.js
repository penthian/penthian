...
const SUMSUB_APP_TOKEN =
  "sbx:8zk25o7qXh5xDJElEVeSU4RU.jlZVmSLwfs6uwMfM679RhE3eHPpQtQ0M"; // Example: sbx:uY0CgwELmgUAEyl4hNWxLngb.0WSeQeiYny4WEqmAALEAiK2qTC96fBad - Please don't forget to change when switching to production
const SUMSUB_SECRET_KEY = "HYvkqWtfVMoFVq0sWtvgsrUgIaYKCElT"; // Example: Hej2ch71kG2kTd1iIUDZFNsO5C1lh5Gq - Please don't forget to change when switching to production
...
// 1) Get applicant data if not found create an applicant then get data
// 2) Getting applicant status
// 3) Getting access tokens for SDKs

router.post("/kyc-verification", async (req, res) => {
  try {
    ...
    externalUserId = walletAddress.toLowerCase();
    levelName = "basic-kyc-level";
    ...
    if (!allData.applicantId) {
      ...
      return res.status(200).json({...allData, message: "applicant id not found" });
    }
    ...
    if (allData.applicantStatus === "completed") {
      ...
      return res.status(200).json({...allData, message: "KYC already completed" });
    }
