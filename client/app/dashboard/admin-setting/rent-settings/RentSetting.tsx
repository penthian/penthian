...
import { Coins, HandCoins, LinkIcon, ListFilter, MapPin, User } from "lucide-react";
...
const RentSetting: React.FC = () => {
  ...
  return (
    <div className="w-full flex flex-col md:flex-row gap-4">
      ...
      {/* Display Skeleton or Product Preview when loading or no product found */}
      {productLoading ? (
        <RentingSkeletonCard />
      ) : !propertyId ? (
        ...
        <div className="w-full overflow-hidden md:px-4 flex items-stretch justify-between xl:flex-row max-w-full flex-col xl:gap-4 gap-10 pb-6">
          ...
          <Card className="xl:w-[55%] w-full flex flex-col h-full">
            <CardContent className="flex flex-col h-full gap-2">
              ...
              <HeadingPara title={metadata?.name} className="text-xl md:!text-2xl" tag="h1" />

              <div className="flex items-center gap-2">
                <div className="flex items-start gap-1">
                  <MapPin className="h-5 w-5 text-grey-2" />
                  <h3 className="text-base font-medium text-grey-6">
                    {metadata?.attributes[0]?.value}, {metadata?.attributes[1]?.value}
                  </h3>
                </div>
              </div>
