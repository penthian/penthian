...
import {
  _changePlatformFeePercentage,
  _getPlatformFeePercentage,
...
const PlatformFee: React.FC = () => {
  ...
  const handleChangePlatformFee = async () => {
    ...
    try {
      ...
      await _changePlatformFeePercentage({
        newPercentage: String(_newPercentage),
        particleProvider,
      });

      await handleFetchPlatformFee();
      NotifySuccess("Platform fee percentage updated successfully.");
      setAmount("");
    } catch (error: any) {
      HandleTxError(error);
    } finally {
  ...
  const handleFetchPlatformFee = async () => {
    try {
      setFeeLoading(true);
      const proposalFee = await _getPlatformFeePercentage();
    ...
    <>
      <Card className="w-full">
        <CardContent className="space-y-6">
          <Card className="rounded-xl">
            <CardContent>
              <div className="space-y-2">
                <p className="text-base 3xl:text-xl text-grey-6">Platform Fee</p>
          ...
          <Button
            className="w-full"
            onClick={handleChangePlatformFee}
            loading={changingFee}
          >
            Update
          </Button>
