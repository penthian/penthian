...
const WhitelistAgent: React.FC = () => {
  ...
  const handleChangeAccess = async () => {
    ...
    if (makeAgent === "not_selected") {
      return NotifyError("Please select an option to make agent or not.");
    }
    ...
    try {
      ...
      await _updateAgentWhitelistStatus({
        _agent: userAddress,
        _makeAgent: makeAgent === "yes",
        particleProvider,
      });
      setUserAddress("");
      setMakeAgent("not_selected");
    ...
    <Card>
      <CardContent className="space-y-6">
        <Card className="rounded-xl">
          <CardContent>
            <div className="space-y-2">
              <p className="text-base 3xl:text-xl text-grey-6">Whitelist Agent</p>
            </div>
          </CardContent>
        </Card>
        <Input
        ...
        <select
          ...
          value={makeAgent}
          onChange={(e) => setMakeAgent(e.target.value as "not_selected" | "yes" | "no")}
        >
          <option value="not_selected">Select Option</option>
          <option value="yes">Make Agent</option>
          <option value="no">Remove Agent</option>
        </select>
